...
int main(int argc, char **argv) {
    ...
    if (argc != 5) {
        printf("Usage : ./hello_classification <path_to_model> <path_to_image> <device_name> <bfloat_flag> \n");
        return EXIT_FAILURE;
    }
    ...
    char* token;
    char* rest = bfloat_flag;
    token = strtok_r(rest, "=", &rest);
    if (strcmp(token, "-enforcebfloat16") == 0) {
        cfg.name = "ENFORCE_BF16";
        token = strtok_r(rest, "=", &rest);
        ...
        status = ie_core_set_config(core, &cfg, "CPU");
        if (status != OK)
            goto err;
    }
    struct ie_param par;
    status = ie_core_get_config(core, "CPU", "ENFORCE_BF16", &par);
    char* enforce_bf16 = par.params;
