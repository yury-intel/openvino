                ...
                StatusCode executeSpecified(std::vector<Blob::Ptr>& inputs, std::vector<Blob::Ptr>& outputs,
                    ...
                    parallel_for(real_rois, [&](int n) {
                        ...
                        if (mode_ == "average") {
                            ...
                            for (int c = 0; c < nc; c++) {
                                for (int h = 0; h < nh; h++) {
                                    for (int w = 0; w < nw; w++, index++) {
                                        dst_data[index] = 0;

                                        int hstart = static_cast<int>(floor(static_cast<float>(h) * bin_size_h + roi_start_h));
                                        ...
                                        int wstart = static_cast<int>(floor(static_cast<float>(w) * bin_size_w + roi_start_w));
                                        ...
                                        if (bin_area) {
                                            ...
                                            const auto *bottom_data =
                                                    src_data + (roi_batch_ind * channels + (gc - blockResidual)) * height * width;
                                            float accum = 0.0f;
                                            ...
                                            for (int hh = hstart * width * block_size; hh < heightIndexBound; hh += blockOffset) {
                                                for (int ww = wstart * block_size; ww < weightIndexBound; ww += block_size) {
                                                    accum += bottom_data[hh + ww + blockResidual];
                                                }
                                            }
                                            accum /= bin_area;
                                ...
                                for (int h = 0; h < nh; h++) {
                                    for (int w = 0; w < nw; w++, index++) {
                                        ...
                                        for (size_t bin_y = 0; bin_y < spatial_bins_y_; bin_y++) {
                                            float box_ymin = roi_start_h + (bin_y) * (roi_height / spatial_bins_y_);
                                            float box_ymax = roi_start_h + (bin_y + 1) * (roi_height / spatial_bins_y_);
                                            for (size_t bin_x = 0; bin_x < spatial_bins_x_; bin_x++) {
                                                float box_xmin = roi_start_w + (bin_x) * (roi_width / spatial_bins_x_);
                                                float box_xmax = roi_start_w + (bin_x + 1) * (roi_width / spatial_bins_x_);

                                                size_t gc = c + (bin_y*spatial_bins_x_ + bin_x)*nc;
                                                const int blockResidual = gc % block_size;
                                                size_t src_idx = (roi_batch_ind * channels + (gc - blockResidual)) * height * width;
                                                const auto *bottom_data = src_data + src_idx;

                                                float height_scale = nh > 1 ? (box_ymax - box_ymin) * (height - 1) / (pooled_height_ - 1) : 0.0f;
                                                float width_scale = nw > 1 ? (box_xmax - box_xmin) * (width - 1) / (pooled_width_ - 1) : 0.0f;
                                    ...
                                    for (int w = 0; w < nw; w++, index++) {
                                        ...
                                        for (size_t ih = 0; ih < spatial_bins_y_; ih++) {
                                            for (size_t iw = 0; iw < spatial_bins_x_; iw++) {
                                                ...
                                                accum += bilinear_interp<inputType>(offset_bottom_data +
                                                                                    c1 * height * width, w1, h1, width, block_size, c1 % block_size);
                                                count++;
                                            }
                                        }
                                        accum = count == 0 ? 0.f : accum / count;
                                        dst_data[index] = accum;
                                    }
                                }
                            }
                    ...
                    const int offset = real_rois * nc * nh * nw;
                    dst_data += offset;
                    parallel_for((nn * nc * nh * nw - offset), [&](int index) {
                        dst_data[index] = 0;
                    });
...
}  // namespace InferenceEnginegit push origin